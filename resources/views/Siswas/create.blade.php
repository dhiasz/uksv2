<x-app-layout>
    <div class="max-w-4xl mx-auto mt-10 bg-white dark:bg-gray-800 p-8 rounded-lg shadow-lg">
        <h1 class="text-2xl font-bold text-gray-800 dark:text-gray-200 mb-6">Tambah Data Siswa</h1>
        <form action="{{ route('siswas.store') }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="number" name="tb" id="tb" class="w-full px-4 py-2 border rounded-lg dark:bg-gray-700 dark:text-gray-200" value="{{ old('tb') }}" required>
                @error('tb')
                    <div class="text-red-500 text-sm mt-1">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="number" name="bb" id="bb" class="w-full px-4 py-2 border rounded-lg dark:bg-gray-700 dark:text-gray-200" value="{{ old('bb') }}" required>
                @error('bb')
                    <div class="text-red-500 text-sm mt-1">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="text" name="tensi" id="tensi" class="w-full px-4 py-2 border rounded-lg dark:bg-gray-700 dark:text-gray-200" value="{{ old('tensi') }}">
                @error('tensi')
                    <div class="text-red-500 text-sm mt-1">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="text" name="goldar" id="goldar" class="w-full px-4 py-2 border rounded-lg dark:bg-gray-700 dark:text-gray-200" value="{{ old('goldar') }}">
                @error('goldar')
                    <div class="text-red-500 text-sm mt-1">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="flex justify-end space-x-4">
                ...
                <button type="submit" class="px-4 py-2 bg-green-500 text-white font-semibold rounded-lg hover:bg-green-600">Simpan</button>
            </div>
        </form>
    </div>
</x-app-layout>
